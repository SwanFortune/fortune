## Autoload. The one place the game's version is written down, and the credits
## that go with it.
##
## Neither existed. A player could not tell one build from another — which
## matters the first time somebody reports a bug and cannot say which version
## they were playing — and nothing in the game said where it came from, which
## for a port of somebody else's prototype is not a detail to leave out.
##
## WHY AN AUTOLOAD AND NOT A CONSTANT IN A SCENE. Three places need the version
## (the main menu, the credits screen, and the base pack's mod.json, which
## declares the version of the content), and a number written down three times
## is a number that is wrong in two of them within a month.
## tests/test_dead_content.gd asserts this and data/base/mod.json agree.
extends Node

## Semantic-ish. MINOR moves when the game changes for the player; PATCH when
## something is fixed without changing what the game is. It is 0.x because the
## content is not finished — see docs/PORTING_NOTES.md for what is missing.
const MAJOR := 0
const MINOR := 2
const PATCH := 0


func string() -> String:
	return "%d.%d.%d" % [MAJOR, MINOR, PATCH]


## Version plus the engine it is running on, for a bug report. Godot's own
## version is read at runtime rather than written here, so it cannot be stale.
func full() -> String:
	var e := Engine.get_version_info()
	var line := "%s · Godot %s.%s.%s" % [string(), e.get("major", "?"), e.get("minor", "?"), e.get("patch", 0)]
	var b := build()
	return line if b == "" else "%s · %s" % [line, b]


## WHICH BUILD THIS IS, as a short commit — "9117492", or "9117492 (modified)",
## or "" when running from source rather than from an export.
##
## MAJOR/MINOR/PATCH above are bumped by hand, so every build between two bumps
## calls itself the same thing. That is not a small problem: an export sitting
## in build/ a day and fifty-four commits old looked, when run, like a broken
## export rather than an old one, and nothing in it could tell the difference.
##
## build.sh writes res://build_stamp.cfg immediately before exporting and
## deletes it after, so it belongs to a build and never to the source tree. Its
## absence is meaningful and is not an error: it means nobody exported this.
const STAMP_PATH := "res://build_stamp.cfg"

func build(path: String = STAMP_PATH) -> String:
	var cfg := ConfigFile.new()
	if cfg.load(path) != OK:
		return ""
	var sha := str(cfg.get_value("build", "commit", ""))
	if sha == "":
		return ""
	return sha + (" (modified)" if bool(cfg.get_value("build", "dirty", false)) else "")


## The same, with the branch and the date, for the credits screen. Empty when
## there is no stamp.
func build_detail(path: String = STAMP_PATH) -> String:
	var cfg := ConfigFile.new()
	if cfg.load(path) != OK:
		return ""
	return "%s · %s · %s" % [
		build(path), str(cfg.get_value("build", "branch", "?")),
		str(cfg.get_value("build", "at", "?")),
	]


## The credits, as [heading, [lines]] pairs — the same shape HowToPlay uses, so
## one renderer does both.
##
## EVERYTHING HERE IS A FACT ABOUT THE REPOSITORY, not a guess. No names are
## invented: the prototype is credited as a file, the engine and the bundled
## font by what they are. Where a person's name belongs, there is a line saying
## so and nothing else — an empty slot is honest, a placeholder name is not.
func credits() -> Array:
	return [
		[I18n.t("THE GAME"), [
			I18n.t("Parlour began as a self-contained browser prototype, Parlour v23.dc.html — the design, the writing, the cards and the numbers are all its author's."),
			I18n.t("This is a port of that prototype to Godot. The rules engine is a direct translation of its simulate(); where the two disagree, the prototype is right."),
			I18n.t("Design and writing: see the repository."),
		]],
		[I18n.t("BUILT WITH"), [
			"· " + I18n.t("Godot Engine %s — MIT licence, godotengine.org") % _godot_version(),
			"· " + I18n.t("The engine's default interface font, Open Sans (Apache 2.0)."),
		]],
		[I18n.t("ART AND SOUND"), _art_and_sound()],
		[I18n.t("TRANSLATION"), _translation_lines()],
		[I18n.t("THIS BUILD"), _this_build()],
	]


## HOW MUCH OF THE ART AND THE AUDIO IS REAL, COUNTED RATHER THAN ASSERTED.
##
## This block used to be three sentences typed by hand, one of which said flatly
## that everything in the build was a placeholder. That was true the day it was
## written and stops being true at the first delivery — and the credits are the
## one screen whose whole job is to state facts about the build, on which nobody
## looks often enough to notice one going stale. So the standing note keeps its
## qualifier ("still marked a placeholder") and the numbers underneath it come
## from the manifests.
##
## The two name lines stay as they are. There is no registry of who drew what, an
## empty slot is honest, and a name is not something to derive.
func _art_and_sound() -> Array:
	return [
		I18n.t("Every card face, portrait and sound in this build that is still marked a placeholder was generated by the project itself, and is meant to be replaced. See docs/ART_GUIDE.md and docs/SOUND_GUIDE.md."),
		"· " + I18n.t("Illustration: unfilled."),
		"· " + I18n.t("Music and sound: unfilled."),
		"· " + I18n.t("Drawings: %s.") % _delivered(Art.status_summary(), Art.UNDELIVERED),
		"· " + I18n.t("Sound and music: %s.") % _delivered(Audio.status_summary(), Audio.UNDELIVERED),
	]


## Turns a by-status count into the one phrase a reader wants. `undelivered` is
## the status meaning "not made yet" — it differs between the two (art is
## "missing", audio is "placeholder"), which is why each half names its own and
## this does not guess.
func _delivered(summary: Dictionary, undelivered: String) -> String:
	var total := 0
	var done := 0
	for st in summary:
		total += int(summary[st])
		if str(st) != undelivered:
			done += int(summary[st])
	if total == 0:
		return I18n.t("nothing registered")
	if done == 0:
		return I18n.t("none of %d delivered") % total
	if done == total:
		return I18n.t("all %d delivered") % total
	return I18n.t("%d of %d delivered") % [done, total]


## The build's own facts. A list rather than a literal because the commit line
## is only there on an EXPORT: run from source there is no stamp, and a line
## reading "unknown" would be noise on a screen nobody is debugging.
func _this_build() -> Array:
	var out: Array = [
		"· " + I18n.t("Version %s") % string(),
		"· " + I18n.t("Engine %s") % _godot_version(),
		"· " + I18n.t("Content packs loaded: %s") % _pack_names(),
	]
	if build_detail() != "":
		out.append("· " + I18n.t("Built from %s") % build_detail())
	return out


func _godot_version() -> String:
	var e := Engine.get_version_info()
	return "%s.%s.%s" % [e.get("major", "?"), e.get("minor", "?"), e.get("patch", 0)]


## Credits every locale that ships, by name, and says plainly how far each one
## has got — a half-finished translation is worth crediting and worth being
## honest about.
func _translation_lines() -> Array:
	var out: Array = []
	var languages := I18n.locales()
	for code in languages:
		if code == "en":
			continue
		var cov: Dictionary = I18n.coverage(str(code))
		var total := int(cov.get("total", 0))
		var done := int(cov.get("translated", 0))
		var pct := 0 if total == 0 else int(round(100.0 * done / total))
		out.append("· %s — %d%%" % [languages[code], pct])
	if out.is_empty():
		out.append(I18n.t("English only."))
	out.append(I18n.t("Translators: unfilled. See docs/LOCALIZATION.md."))
	return out


## Every pack that actually loaded, named. Puts a modded build's provenance in
## the credits, which is where someone filing a bug will be asked to look.
func _pack_names() -> String:
	var names: Array = []
	for p in Content.packs:
		if bool(p.get("enabled", true)):
			names.append("%s %s" % [p.get("name", p.get("id", "?")), p.get("version", "")])
	return ", ".join(names) if not names.is_empty() else "—"
